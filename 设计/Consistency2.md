...
第一步，订单服务创建一个带有NEW状态的Order （订单），发布了一个“Order Created Event（创建订单）”的事件。

![](consistency2.png)

第二步，客户服务消费Order Created Event事件，为此订单预留信用，发布“Credit Reserved Event（信用预留）”事件

![](consistency3.png)

第三步，订单服务消费Credit Reserved Event，改变订单的状态为OPEN
