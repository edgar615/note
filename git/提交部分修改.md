...
有时候在提交代码时并不想使用 `git add .`命令将所有修改都提交。可以使用交互式暂存的方式.
...
git add -i
```
这个命令的输出分为两部分。前面是发生了修改的文件，后面是可以使用的命令，类似于
...
## 保存新文件
update只能保存发生修改的文件，不能保存新加的文件，新建的文件需要使用命令4来保存
